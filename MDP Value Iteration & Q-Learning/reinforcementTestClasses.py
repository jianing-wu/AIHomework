...
class ValueIterationTest(testClasses.TestCase):
    ...
    def execute(
            ...
            moduleDict,
            solutionDict,
            vocareumLeaderBoardScoreFile=None,  # pylint: disable = unused-argument
    ):
...
class EvalAgentTest(testClasses.TestCase):
    ...
    def execute(
        ...
        # Only works because bonus2 comes after q7 and bonus1, so we just
        # overwrite what they wrote (they use this same class).
        if vocareumLeaderBoardScoreFile is not None:
            with open(vocareumLeaderBoardScoreFile, "w") as file:
                file.write(f"bonus2 wins, {wins}\n")
...
def parseGrid(string):
    grid = [
        [entry.strip() for entry in line.split()] for line in string.split('\n')
    ]
    for row in grid:
