...
class QLearningAgent(ReinforcementAgent):
    ...
    def getAction(self, state):
        ...
        action otherwise.  Note that if there are no legal actions, which is the
        case at the terminal state, you should choose None as the action.
        ...
        action = None
        if legalActions:
            actRandomly = util.flipCoin(self.epsilon)
            if actRandomly:
...
class PacmanQAgent(QLearningAgent):
    """Exactly the same as QLearningAgent, but with different default
    parameters."""
    ...
    def __init__(self,
        ...
        line. For example, to change the exploration rate, try: python
        pacman.py.

        -p PacmanQLearningAgent -a epsilon=0.1.
