...
class SuperFoodProblemTest(testClasses.TestCase):

    def __init__(self, question, testDict):
        super(SuperFoodProblemTest, self).__init__(question, testDict)
        self.layoutText = testDict['layout']
        self.layoutName = testDict['layoutName']
    ...
    def solution(self, search, searchAgents):
        ...
        gameState = pacman.GameState()
        gameState.initialize(lay, 0)
        problem = searchAgents.SuperFoodSearchProblem(gameState)
        path = search.bfs(problem)
        ...
        for x in range(1, right + 1):
            for y in range(1, top + 1):
                cellChar = gameState.getCellContent(x, y)
                if cellChar in [',', '.', ';'] and (x, y) in visited:
                    foodEaten[cellChar] += 1
        #print(path)
        #print(foodEaten)
        return path, visited, foodEaten

    def execute(self, grades, moduleDict, solutionDict, vocareumLeaderBoardScoreFile = None):
        search = moduleDict['search']
        searchAgents = moduleDict['searchAgents']
        gold_length = int(solutionDict['solution_length'])
        solution, visited, foodEaten = self.solution(search, searchAgents)
        ...
        for x, y in visited:
            if gameState.hasWall(x, y):
                grades.addMessage('FAIL: %s' % self.path)
                grades.addMessage('Pacman crashes into the wall at (%d, %d)' % (x, y))
                return False

        for foodtype in [',', '.', ';']:
            if foodEaten[foodtype] < 2:
                grades.addMessage('FAIL: %s' % self.path)
                grades.addMessage('Eaten only %d of food '%(foodEaten[foodtype]) + foodtype)
                return False
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        path, _, _ = self.solution(search, searchAgents)
        length = len(path)
        print("Problem solved")
...
class SuperFoodHeuristicSanity(testClasses.TestCase):

    def __init__(self, question, testDict):
        super(SuperFoodHeuristicSanity, self).__init__(question, testDict)
        self.layout_text = testDict['layout']
    ...
    def execute(self, grades, moduleDict, solutionDict, vocareumLeaderBoardScoreFile = None):
        ...
        lay = layout.Layout([l.strip() for l in self.layout_text.split('\n')])
        game_state.initialize(lay, 0)
        problem = searchAgents.SuperFoodSearchProblem(game_state)
        start_state = problem.getStartState()
        #print(lay)
        #print("True cost: ", float(solutionDict['cost']))
        h0 = searchAgents.superFoodHeuristic(start_state, problem)
        succs = problem.getSuccessors(start_state)
        #print("Start state: ", start_state, " h = ", h0)
        # cornerConsistencyA
        for succ in succs:
            h1 = searchAgents.superFoodHeuristic(succ[0], problem)
            #print("Succ ", succ[0], " h = ", h1)
            if h0 - h1 > 1:
                grades.addMessage('FAIL: inconsistent heuristic')
                return False
        heuristic_cost = searchAgents.superFoodHeuristic(start_state, problem)
        true_cost = float(solutionDict['cost'])
        # cornerNontrivial
        if heuristic_cost == 0:
            grades.addMessage('FAIL: must use non-trivial heuristic')
        ...
        heuristics = []
        for state in states:
            heuristics.append(searchAgents.superFoodHeuristic(state, problem))
        for i in range(0, len(heuristics) - 1):
            h0 = heuristics[i]
            h1 = heuristics[i+1]
            # cornerConsistencyB
    ...
    def writeSolution(self, moduleDict, filePath):
        ...
        start_state = pacman.GameState()
        start_state.initialize(lay, 0)
        problem = searchAgents.SuperFoodSearchProblem(start_state)
        solution = search.astar(problem, searchAgents.superFoodHeuristic)
        handle.write('cost: "%d"\n' % len(solution))
        handle.write('path: """\n%s\n"""\n' % wrap_solution(solution))
        handle.close()
        return True
...
class CornerHeuristicPacman(testClasses.TestCase):
    ...
    def execute(self, grades, moduleDict, solutionDict, vocareumLeaderBoardScoreFile = None):
        ...
        #gold_length = int(solutionDict['cost'])
        #solution, missedCorners = self.solution(search, searchAgents)
...
# todo change this to not output threshold-based scores
# python3 autograder.py --generate-solutions
#   **** change CONFIGq8 to CONFIG to not override existing solutions
#   **** remove depends statment in q8/CONFIG
class SuperFoodHeuristicPacman(testClasses.TestCase):

    def __init__(self, question, testDict):
        super(SuperFoodHeuristicPacman, self).__init__(question, testDict)
        self.layout_text = testDict['layout']
    ...
    def execute(self, grades, moduleDict, solutionDict, vocareumLeaderBoardScoreFile = None):
        ...
        if searchAgents.superFoodHeuristic(start_state, problem) > true_cost:
            grades.addMessage('FAIL: Inadmissible heuristic')
            return False
        path = search.astar(problem, searchAgents.superFoodHeuristic)
        print("path:", path)
        print("path length:", len(path))
        cost = problem.getCostOfActions(path)
        if cost > true_cost:
