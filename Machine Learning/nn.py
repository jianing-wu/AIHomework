...
class FunctionNode(Node):
    ...
    def _forward(self, *inputs):
        raise NotImplementedError("Should not be calling FunctionNode directly, try using a derived class")
...
class Subtract(FunctionNode):
    """
    Subtracts matrices element-wise.

    Usage: nn.Subract(x, y)
    Inputs:
        x: a Node with shape (batch_size x num_features)
        y: a Node with the same shape as x
    Output:
...
class ScalarMatrixMultiply(FunctionNode):
    """
    Multiplies all elements of a matrix by a scalar.

    Usage: nn.ScalarMatrixMultiply(s, x)
    Inputs:
        s: a Node with a single scalar value
        x: a Node with shape (batch_size x num_features)
    Output:
        a Node with shape (batch_size x num_features)
    """
    ...
    def _forward(*inputs):
        ...
        assert inputs[0].size == 1, (
            "First input should have size 1, instead has {}".format(
                inputs[0].size))
        assert inputs[1].ndim == 2, (
            "Second input should have 2 dimensions, instead has {}".format(
                inputs[1].ndim))
    ...
    def _backward(gradient, *inputs):
        raise NotImplementedError(
            "Backpropagation through ScalarMatrixMultiply nodes is not needed in this "
            "assignment")
...
class Sigmoid(FunctionNode):
    ...
    def _forward(*inputs):
        ...
        assert inputs[0].ndim == 2, (
            "Input should have 2 dimensions, instead has {}".format(
                inputs[0].ndim))
        return Sigmoid.sigmoid(inputs[0])

    @staticmethod
    def _backward(gradient, *inputs):
        assert gradient.shape == inputs[0].shape
        return [Sigmoid.sigmoid(inputs[0]) * (1 - Sigmoid.sigmoid(0))]
